...
`plugins/heuriboost/skills/heuriboost-rag/templates/feature_recipes.yaml`；Python 实现在
`plugins/heuriboost/skills/heuriboost-rag/scripts/features/`（`registry.py`、`primitives.py`、
`recipes.py`）。
...
当前 V0 feature set 有 12 个 baseline features：

- Retriever 先验：`dense_score`、`dense_rank_inverse`、`sparse_score`、
...
- 长度 / 信息密度：`low_information_density_flag`、`doc_length_log`、
  `query_length_log`。

这就是消融里说的 "baseline 12"。Candidate Feature 只在消融时作为第 13 个特征
临时追加；除非人工显式 promote，否则不会进入 shipped feature set。
...
- 一个 trial 是一次 Optuna objective evaluation：采样一组 XGBoost 参数，在固定
  train/validation 快照上训练并打分。
- `--n-trials N` 是参数采样预算，不是特征个数。若触发 timeout 或中断，实际完成数
  可能少于请求值。
- 当前搜索空间固定为 7 个 XGBoost 参数：`max_depth`、`min_child_weight`、`eta`、
  `subsample`、`colsample_bytree`、`gamma`、`reg_lambda`。
- 特征数变多可能需要更高预算，因为 objective landscape 会变；但公平对比时不能让
  一侧拿到更大的 `n_trials`。
- 实用预算：`1` smoke、`5` 快速检查、`20` V0 默认、`50+` 用于更大的 feature set
  或不稳定结果。
...
`--n-trials N` 分别作用在两个 HPO cell：B 对 baseline 12 跑 N 次 trial，D 对
baseline 12 + candidate 跑 N 次 trial。A/C 使用固定参数，不跑 HPO。所以每个候选的
成本约为 `2N` 次 HPO trial；auto-ablation 若有 `C` 个有效候选，总成本约为
`C * 2N`。例如 5 个候选配 `--ablation-n-trials 20`，大约请求
`5 * (20 + 20) = 200` 次 HPO trial。B 和 D 必须保持相同 `n_trials` 与 `seed`，
否则 D-B 会被搜索预算污染，不能代表候选特征本身的贡献。
...
如果多个候选都得到 `promote` 推荐，最多先合入一个，然后重新训练/评估，再考虑下一个。
不要批量合入候选特征。
...
`failure_analysis.md` + 现有特征集，调一次 LLM（DeepSeek，JSON 模式）为整个
discovery round 提议最多 5 个候选特征，静态校验后写入候选文件，供
`run_ablation.py` 消费。
...
`candidates/<name>/{recipe.yaml, impl.py}` + `candidates_report.md`（含 review 或
auto-ablation 警告）。
...
candidate` + import 白名单），生成阶段**不 importlib 加载**（不可信）。默认路径是
用户审阅 `impl.py` 后，用 `run_ablation.py` 测试候选。

如需 discovery 后自动进入消融，传 `--auto-ablate`：
...
Auto-ablation 会对每个有效候选各跑一次 `run_ablation.py` 子进程。每个候选输出到
`<ablation-output-dir>/<candidate>/ablation/`，discovery 额外写出
`<out-dir>/auto_ablation_report.md`。它是 fail-fast：某个候选失败后，后续候选不再运行。
多个 `promote` 推荐会原样报告，不自动选 winner。promotion 仍然手动。
`--ablation-n-trials N` 是每个候选各自的预算，并直接映射到
`run_ablation.py --n-trials N`。

`--n-candidates` 是所有 pending cases 共享的 round 预算，不是每个 case 一个预算。
CLI 接受 `1..5`。

Discovery 使用配置的 regression case 文件里所有 `status: pending` 的 cases。
当前没有 case 子集过滤参数。

如果构造后的 prompt 超过本地 24,000 字符预算，discovery 会在调用 LLM 前失败，
不会自动截断 pending cases。
...
Codex-compatible skill 位于 `plugins/heuriboost/skills/heuriboost-rag/SKILL.md`，有三种模式：
