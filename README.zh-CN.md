...
`skills/reckless-input-builder/` 里的 AI 友好输入构建规范。它告诉 agent 如何把
检索候选、accepted/rejected 文档、rank、score、用户反馈映射成这两张表，如何判断
full / weak 验收，以及如何用 `compile_cases.py` 校验结果。
...
└── skills/reckless-input-builder/    构建 reckless 输入文件的 AI 友好规范
```
